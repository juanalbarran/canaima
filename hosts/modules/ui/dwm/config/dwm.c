...
void get_battery_info(char *buffer, int size) {
  FILE *fp;
  int capacity = 0;
  char status[20] = {0};
  const char *icon = "󰁺"; // Default empty
  ...
  if (fp) {
    ...
    snprintf(buffer, size, ""); // No battery found
    return;
  }
  ...
  /* Determine Icon based on your Waybar config */
  if (strcmp(status, "Charging") == 0) {
    icon = "󰂄";
  } else {
    /* Map capacity to icons: 0-10, 11-20, etc. */
    const char *icons[] = {"󰁺", "󰁺", "󰁻", "󰁼", "󰁽",
                           "󰁿", "󰂀", "󰂁", "󰂂", "󰁹"};
    int idx = capacity / 10;
    if (idx > 9)
      idx = 9;
  ...
  /* Format the string */
  snprintf(buffer, size, "%s %d%%", icon, capacity);
}
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    ...
    // Battery logic
    char battStr[64];
    get_battery_info(battStr, sizeof(battStr));

    int battw = 0;
    if (strlen(battStr) > 0) {
      battw = TEXTW(battStr) - lrpad + 2;
      /* Logic to change color based on status (Charging = Blue/Sel, Low = Norm)
       */
      /* Note: To get specific Red/Yellow requires defining new schemes,
         using SchemeSel (Blue) for charging to match your Waybar preference */
      int schemeId = (strstr(battStr, "󰂄")) ? SchemeSel : SchemeNorm;

      drw_setscheme(drw, scheme[schemeId]);
      // Draw Battery: Position it to the left of stext (tw) and the margin (35)
      drw_text(drw, m->ww - TEXTW(stext) - 35 - battw + lrpad, 0, battw, bh, 0,
               battStr, 0);
    }
    ...
    /* Add 35 to tw so the window title bar stops earlier and respects the
     * margin */
    tw += 35;
  }
...
void runorraise(const Arg *arg) {
  ...
  const char *target_class = argv[0];
  Client *c;
  Monitor *m;
  XClassHint ch = {NULL, NULL};

  /* 1. Search for the window across all monitors */
  for (m = mons; m; m = m->next) {
    for (c = m->clients; c; c = c->next) {
      XGetClassHint(dpy, c->win, &ch);
      /* Check if class matches */
      if ((ch.res_class && strcmp(ch.res_class, target_class) == 0) ||
          (ch.res_name && strcmp(ch.res_name, target_class) == 0)) {
        ...
        /* Switch to the tag where the window is, if not visible */
        if (!ISVISIBLE(c)) {
          view(&((Arg){.ui = c->tags}));
        }
  ...
  /* 2. If not found, spawn the command (skip the first arg which is the class)
   */
  Arg a = {.v = argv + 1};
  spawn(&a);
}
...
void movestack(const Arg *arg) {
  ...
  if (c && c != selmon->sel) {
    ...
    selmon->sel->next = c->next == selmon->sel ? selmon->sel->next : c->next;
    c->next = temp;
